...
class PendingIbExport implements FromQuery, WithHeadings, WithMapping
{
    use Exportable;
    ...
    {
        $filters = $this->request->only(['global_search', 'phone', 'country', 'status', 'created_at', 'tag', 'date_filter', 'ib_group']);
        $balanceStatus = $this->request->balanceStatus;

        $query = User::with('ibGroup')
