...
//            dd($today);
            ...
            if ($profitOffDays == null || !in_array($today, $profitOffDays)) {
                ...
                $rate = $invest->interest;
                if ($invest->interest_type == 'percentage') {
//                    dd($rate);
                    $profitDeduction = ProfitDeduction::whereDate('start_date', $date)->orderBy('id', 'ASC')->first();
//                   dd($profitDeduction);
                    if ($profitDeduction) {
                        $deduction_in_percentage = round((($profitDeduction->percentage / 100) * ($rate / 100)) * 100, 2);
                        $rate = $rate - $deduction_in_percentage;
                        if ($rate < $invest->min_interest) {
                            $rate = $invest->min_interest;
                        }
                    }
                }
//                dd($rate,$invest->invest_amount);
                $calculateInterest = ($rate * $invest->invest_amount) / 100;
//                    dd($calculateInterest);
                ...
                $updateData = [
                    ...
                    'profit_hold_amount' => ($invest->profit_hold_amount + $interest),
...
//                        $user->increment('profit_balance', $interest);
    ...
    public function profitClearCronJob()
    {

        $ongoingInvestment = Invest::whereIn('status', [InvestStatus::Ongoing, InvestStatus::Completed])->where('next_profit_clear_time', '<=', Carbon::now()->format('Y-m-d H:i:s'))->cursor();
//dd($ongoingInvestment);
        foreach ($ongoingInvestment as $invest) {
//            dd($invest);
            $date = $invest->next_profit_clear_time;
            if (!$date) {
                $date = Carbon::now();
            } else {
                $date = Carbon::parse($invest->next_profit_clear_time);
            }
            $user = User::find($invest->user_id);

            $user->increment('profit_balance', $invest->profit_hold_amount);
            $nextProfitTime = $date->addHour($invest->profit_clear_hours);

            $updateData = [
                'next_profit_clear_time' => $nextProfitTime,
                'profit_hold_amount' => 0,
