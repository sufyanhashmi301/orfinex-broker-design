...
class PricingSubscription
{
    ...
    public function getScheme(): PricingScheme
    {
        return $this->scheme;
    }

    public function setScheme(PricingScheme $scheme): self
    {
        $this->scheme = $scheme;
  ...
  public function setDaysToPassAmount(float $days_to_pass_amount): self
   ...
   public function setPayoutAmount(float $payout_amount): self
    {
   ...
   public function setInput(array $input): self
    ...
    {
        $except = ['id', 'desc', 'slug', 'created_at', 'updated_at'];
    ...
    {
        return to_minus(BigDecimal::of($this->getInvest())->plus($this->getLeverageAmount())->plus($this->getDaysToPassAmount())->plus($this->getProfitSplitAmount())->plus($this->getPayoutAmount())->plus($this->getSwapFreeAmount())  , $this->getDiscountAmount());
    }
...
//        dd($this->getInput()['leverage']);
        return [
            ...
            "pricing_scheme_id" => $this->scheme->id,
            "amount" => $this->getInvest(),
            "amount_allotted" => $this->scheme->amount_allotted,
...
//            "term_end" => $termEnd,
            "meta" => array('payment_type' => $this->getInput()['type'],'source' => $this->source,'fees' => 0, 'exchange' => 0),
