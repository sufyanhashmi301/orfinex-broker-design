                    ...
                    <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700" id="dataTable">
                        ...
                        <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                          @foreach($accounts as $account)
                              ...
                              @php
                                  // Phase started
                                  if($phaseData['type'] == \App\Enums\AccountTypePhase::FUNDED && isset($account->contract['status']) && $account->contract['status'] != \App\Enums\ContractStatusEnums::SIGNED) {
                                  ...
                                  if(isset($account->contract['status'])) {
                                      if($account->contract['status'] == \App\Enums\ContractStatusEnums::PENDING) {
                                          $contract_pending = true;
                                      } elseif ($account->contract['status'] == \App\Enums\ContractStatusEnums::EXPIRED) {
                              ...
                              <tr>
                                ...
                                <td class="table-td">{{ $account->user->first_name . ' ' . $account->user->last_name . ' ('  . $account->user->email . ')'  }}</td>
                                ...
                                <td class="table-td"><span class="badge bg-primary" style="color: #fff">{{ $contract_pending == true ? 'Pending' : $account->status  }}</span></td>
                                <td class="table-td">
                                    @if (
                                        ...
                                        @if($contract_pending == true)
                                            <a href="{{ route('admin.manage-contracts.index') }}" class="inline-flex justify-center">
                                        ...
                                        @elseif (isset($stats) && count($hourly_stats) != 0 )
                                            <a href="{{ route('admin.account.trading_stats', ['account_id' => $account->id ]) }}" target="__blank" class="inline-flex justify-center">
                                                <span class="flex items-center">
                                    ...
                                    @else 
                                        @if ($account->status == \App\Enums\InvestmentStatus::PENDING)
                                          <a href="{{ route('admin.deposit.manual.pending') }}" class="inline-flex justify-center">
                                            <span class="flex items-center">
