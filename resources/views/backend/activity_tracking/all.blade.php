                ...
                <div class="flex-1 input-area relative">
                    <select name="action" id="action" class="form-control w-full h-9">
                        ...
                        <optgroup label="{{ __('Authentication') }}">
                            <option value="user_login" {{ request('action') == 'user_login' ? 'selected' : '' }}>{{ __('User Login') }}</option>
                            <option value="user_logout" {{ request('action') == 'user_logout' ? 'selected' : '' }}>{{ __('User Logout') }}</option>
                        ...
                        <optgroup label="{{ __('Financial') }}">
                            <option value="deposit" {{ request('action') == 'deposit' ? 'selected' : '' }}>{{ __('Deposit') }}</option>
                            ...
                            <option value="withdraw" {{ request('action') == 'withdraw' ? 'selected' : '' }}>{{ __('Withdraw') }}</option>
                            ...
                            <option value="transfer" {{ request('action') == 'transfer' ? 'selected' : '' }}>{{ __('Transfer') }}</option>
                            <option value="transfer_approved" {{ request('action') == 'transfer_approved' ? 'selected' : '' }}>{{ __('Transfer Approved') }}</option>
                            <option value="transfer_rejected" {{ request('action') == 'transfer_rejected' ? 'selected' : '' }}>{{ __('Transfer Rejected') }}</option>
                        </optgroup>
                        ...
                        <optgroup label="{{ __('User Management') }}">
                            <option value="user_register" {{ request('action') == 'user_register' ? 'selected' : '' }}>{{ __('User Register') }}</option>
                            <option value="email_verification" {{ request('action') == 'email_verification' ? 'selected' : '' }}>{{ __('Email Verification') }}</option>
                            ...
                            <option value="get_password" {{ request('action') == 'get_password' ? 'selected' : '' }}>{{ __('Get Password') }}</option>
                        </optgroup>
                        ...
                        <optgroup label="{{ __('Admin Actions') }}">
                            <option value="settings_update" {{ request('action') == 'settings_update' ? 'selected' : '' }}>{{ __('Settings Update') }}</option>
                            <option value="admin_create" {{ request('action') == 'admin_create' ? 'selected' : '' }}>{{ __('Admin Create') }}</option>
                            <option value="admin_update" {{ request('action') == 'admin_update' ? 'selected' : '' }}>{{ __('Admin Update') }}</option>
                            <option value="admin_delete" {{ request('action') == 'admin_delete' ? 'selected' : '' }}>{{ __('Admin Delete') }}</option>
                        </optgroup>
                        <optgroup label="{{ __('Security') }}">
                            <option value="suspicious_activity" {{ request('action') == 'suspicious_activity' ? 'selected' : '' }}>{{ __('Suspicious Activity') }}</option>
                            <option value="account_locked" {{ request('action') == 'account_locked' ? 'selected' : '' }}>{{ __('Account Locked') }}</option>
                            <option value="ip_blocked" {{ request('action') == 'ip_blocked' ? 'selected' : '' }}>{{ __('IP Blocked') }}</option>
                        </optgroup>
                    </select>
                </div>
                <div class="flex-1 input-area relative">
                        ...
                        <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" class="table-th">{{ __('Actor Type') }}</th>
        ...
        (function ($) {
            ...
            .on('processing.dt', function (e, settings, processing) {
                ...
                columns: [
                    ...
                    {data: 'actor_type', name: 'actor_type'},
