    ...
    <div class="max-w-5xl mx-auto">
        <div class="flex justify-between flex-wrap items-center mb-6">
            <h4 class="font-medium text-xl capitalize text-slate-500 dark:text-slate-400 inline-block ltr:pr-4 rtl:pl-4 mb-1 sm:mb-0">
                {{ __('Branch Form') }} — {{ $branch->name }}
            </h4>
            <div class="flex sm:space-x-4 space-x-2 sm:justify-end items-center rtl:space-x-reverse">
                <a href="{{ route('admin.branches.index') }}" class="btn btn-sm btn-primary inline-flex items-center justify-center">
        ...
        <div class="card">
            <div class="card-body p-6">
                <form action="{{ route('admin.branches.form.update', $branch->id) }}" method="post" class="space-y-4">
                    @csrf

                    <div class="input-area">
                        <label class="form-label">
                            <span class="shift-Away inline-flex items-center gap-1" data-tippy-content="Select one or more countries for this branch">
                                {{ __('Countries') }}
                        ...
                        <div class="relative">
                            <select name="countries[]" id="countriesSelect" class="select2 form-control w-full" multiple data-placeholder="Select Countries">
                                @php
                                    $selected = old('countries', $selectedCountryCodes ?? []);
                                @endphp
                        ...
                        <div class="input-area">
                            <div class="flex items-center space-x-7 flex-wrap">
                                <label class="form-label !w-auto">
                                    <span class="shift-Away inline-flex items-center gap-1" data-tippy-content="Enable or disable this Branch form">
                                        {{ __('Status') }}
                                        <iconify-icon icon="mdi:information-slab-circle-outline" class="text-[16px]"></iconify-icon>
                                    </span>
                                </label>
                                <div class="form-switch ps-0">
                                    ...
                                    <label class="relative inline-flex h-6 w-[46px] items-center rounded-full transition-all duration-150 cursor-pointer">
                                        <input type="checkbox" name="status" value="1" class="sr-only peer" {{ old('status', $form->status ?? true) ? 'checked' : '' }}>
...
@section('script')
    <script>
        $(document).ready(function () {
            var i = {{ isset($fields) ? count($fields) : 0 }};
            "use strict";
