...
@section('title')
    {{ __('Pending Branch Form Submissions') }}
@endsection
@section('content')
    ...
    @php
        ...
        if (request()->routeIs('admin.branch-form-submissions.approved')) { $exportStatus = 'approved'; }
        if (request()->routeIs('admin.branch-form-submissions.rejected')) { $exportStatus = 'rejected'; }
    @endphp
    <form id="filter-form" method="POST" action="{{ route('admin.branch-form-submissions.export', $exportStatus) }}">
        @csrf
        <div class="flex flex-col sm:flex-row justify-between flex-wrap gap-3">
            <div class="flex-1 w-full flex flex-col sm:flex-row sm:gap-3 gap-2">
                ...
                <div class="flex-1 input-area relative">
                    <input type="text" name="branch" id="branch" class="form-control" placeholder="Search by Branch Name">
                </div>
            ...
            <div class="overflow-x-auto -mx-6 dashcode-data-table">
                <div class="inline-block min-w-full align-middle">
                    <div class="overflow-hidden ">
                        <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700" id="submissionsTable">
                            <thead>
                                <tr>
    ...
    <!-- Modal for Submission Action -->
    <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="submission-action-modal" tabindex="-1" aria-labelledby="submission-action-modal" aria-hidden="true">
        <div class="modal-dialog modal-lg top-1/2 !-translate-y-1/2 relative max-w-xl w-full pointer-events-none">
            <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white dark:bg-dark bg-clip-padding rounded-md outline-none text-current">
                <div class="modal-body popup-body">
                    <div class="popup-body-text submission-action"></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    (function ($) {
        ...
        .on('processing.dt', function (e, settings, processing) {
            ...
            autoWidth: false,
            ajax: {
                url: "{{ route('admin.branch-form-submissions.data', 'pending') }}",
                data: function (d) {
                    d.search = $('#search').val();
                    d.branch = $('#branch').val();
        ...
        $(document).on('branchFormSubmissionActionCompleted', function() {
