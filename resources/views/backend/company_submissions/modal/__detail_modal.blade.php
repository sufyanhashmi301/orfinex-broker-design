...
<div class="p-4">
    ...
    <div class="flex justify-end gap-2 mt-6">
        <form class="inline" method="post" action="{{ route('admin.company-form-submissions.update-status') }}" onsubmit="return submitCompanySubmissionAction(event, this)">
            @csrf
...
<script>
    function submitCompanySubmissionAction(e, formEl) {
        e.preventDefault();
        var $form = $(formEl);
        var action = $form.find('input[name="status"]').val();
        $.post($form.attr('action'), $form.serialize(), function(resp) {
            if (resp && resp.success) {
                $('#submission-action-modal').modal('hide');
                $(document).trigger('companyFormSubmissionActionCompleted', [{ status: action }]);
            }
        }).fail(function(){
            $(document).trigger('companyFormSubmissionActionCompleted', [{ status: 'error' }]);
        });
        return false;
    }
</script>
