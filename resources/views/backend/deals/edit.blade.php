            ...
            <div class="card-body p-6">
                <div class="grid md:grid-cols-2 grid-cols-1 gap-5">
                    <div class="input-area">
                        ...
                        <select name="lead_id" id="lead_id" class="select2 form-control">
                            ...
                            @foreach($leads as $lead)
                                <option value="{{ $lead->id }}" @if($deal->lead_id == $lead->id) selected @endif>
                                    {{ ucfirst($lead->salutation).' '.$lead->first_name.' '.$lead->last_name }}
                                </option>
                            @endforeach
                        </select>
                        @error('lead_id')
                    ...
                    <div class="input-area">
                        ...
                        <select name="lead_pipeline_id" id="pipelineData" class="select2 form-control">
                            ...
                            @foreach($pipelines as $pipeline)
                                <option value="{{ $pipeline->id }}" @if($deal->lead_pipeline_id == $pipeline->id) selected @endif>
                                    {{ $pipeline->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-area">
                        ...
                        <select name="pipeline_stage_id" id="stages" class="form-control">
                            <option value="">{{ __('select stage') }}</option>
                            @foreach($stages as $stage)
                                <option value="{{ $stage->id }}" data-color="{{ $stage->label_color }}" @if($deal->pipeline_stage_id == $stage->id) selected @endif>
                                    {{ $stage->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-area">
                        ...
                        <div class="joint-input relative">
                            <input type="text" class="form-control !pr-12" oninput="this.value = validateDouble(this.value)" name="value" value="{{ $deal->value }}">
                    ...
                    <div class="input-area">
                        ...
                        <input type="text" name="close_date" class="form-control py-2 flatpickr flatpickr-input" value="{{ $deal->close_date }}" readonly>
                        @error('close_date')
                            <span class="error">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <ul class="contact-details__list space-y-3">
                    ...
                    <li class="text-sm text-slate-600 dark:text-slate-300 py-2">
                        <div class="flex justify-between">
                            ...
                            <span class="w-2/3">{{ $deal->lead->office_phone_number }}</span>
                        </div>
                    </li>
                    <li class="text-sm text-slate-600 dark:text-slate-300 py-2">
                        <div class="flex justify-between">
                            <span class="font-medium">{{ __('Website:') }}</span>
                            <span class="w-2/3">{{ $deal->lead->website }}</span>
                        </div>
                    </li>
                    <li class="text-sm text-slate-600 dark:text-slate-300 py-2">
                        <div class="flex justify-between">
                            <span class="font-medium">{{ __('Address:') }}</span>
                            <span class="w-2/3">{{ $deal->lead->address }}</span>
                        </div>
                    </li>
                </ul>
            </div>
