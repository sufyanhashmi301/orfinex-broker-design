    ...
    <div class="innerMenu card p-4 mb-5">
        <ul class="nav nav-pills flex items-center overflow-x-auto list-none pl-0 pb-1 sm:pb-0 gap-4 menu-open">
            @canany(['deposit-list','deposit-action'])
                <li class="nav-item">
                    <a href="{{ route('admin.deposit.manual.pending') }}" class="nav-link block font-medium font-Inter text-sm leading-tight capitalize text-nowrap rounded-md px-6 py-3 focus:outline-none focus:ring-0 dark:bg-slate-900 dark:text-slate-300 {{ isActive('admin.deposit.manual.pending') }}">
                        {{ __('Pending Payments') }}
                    </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.deposit.history') }}" class="nav-link block font-medium font-Inter text-sm leading-tight capitalize text-nowrap rounded-md px-6 py-3 focus:outline-none focus:ring-0 dark:bg-slate-900 dark:text-slate-300 {{ isActive('admin.deposit.history') }}">
                        {{ __('Payment History') }}
                    </a>
                </li>
            @endcanany
