...
@section('page-title')
    <div class="flex justify-between flex-wrap items-center mb-6">
        ...
        @can('deposit-method-create')
            <div class="flex sm:space-x-4 space-x-2 sm:justify-end items-center rtl:space-x-reverse">
                @isset($button)
                    <a href="{{ $button['route'] }}" class="btn btn-sm btn-primary inline-flex items-center justify-center"
                        type="button">
            ...
            <div class="card border hover:shadow-lg">
                <div class="card-header items-center noborder !p-4">
                    <img class="inline-block h-10"
                        src="{{ isset($method->gateway_id) ? $method->gateway->logo : asset($icon) }}"
                        alt="{{ $method->name }}" />
                    @can('deposit-method-action')
                        <div class="dropdown relative">
                            ...
                            <ul
                                class="dropdown-menu min-w-[120px] absolute text-sm text-slate-700 dark:text-white hidden bg-white dark:bg-slate-700 shadow z-[2] overflow-hidden list-none text-left rounded-lg mt-1 m-0 bg-clip-padding border-none lrt:origin-top-right ">
                                <li>
                                    <a href="{{ route('admin.deposit.method.edit', ['type' => strtolower($type), 'id' => $method->id]) }}"
                                ...
                                <li>
                                    <a href="javascript:void(0);"
                                        class="{{ $method->gateway_code === 'voucher' ? 'disabled' : 'delete-method' }} text-slate-600 dark:text-white block font-Inter font-normal px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white"
                ...
                <div class="card-body p-4 pt-2">
                    ...
                    <ul class="space-y-3">
                        <li class="flex items-center justify-between text-sm">
                            ...
                            <span class="capitalize">{{ $method->processing_time }}</span>
                        </li>
                        <li class="flex items-center justify-between text-sm">
                        ...
                        <li class="flex items-center justify-between text-sm">
                            ...
                            <span>{{ $method->minimum_deposit }} - {{ $method->maximum_deposit }}
                                {{ $method->currency }}</span>
                        </li>
                        <li class="flex items-center justify-between text-sm">
