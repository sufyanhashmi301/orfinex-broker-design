...
@endphp
@section('filters')
    <form id="filter-form" method="POST" action="{{ route('admin.ib.export',['type' => 'all']) }}">
        @csrf
            ...
            <div class="flex-1 w-full flex flex-col sm:flex-row sm:gap-3 gap-2">
                ...
                <div class="flex-1 input-area relative">
                    <select name="ib_group" id="ib_group" class="form-control h-full select2" data-placeholder="{{ __('Select IB Group') }}">
                        ...
                        <option value="no_filter">{{ __('Select IB Group') }}</option>
...
@section('script')
    <script>
        (function ($) {
            ...
            $('#global_search, #ib_group, #ib-bonus-date-filter, #tag').on('change keyup', function() {
                table.draw();
            });
