...
@section('title', __('Pricing Invested Plan'))
...
@section('content')
    <div class="nk-content-body">
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title">{{ __('Pricing Invested Plans') }}</h3>
                    <p>{!! __('Total :count entries.', ['count' => '<span class="text-base">'.$investments->total().'</span>' ]) !!}</p>
                </div>
                <div class="nk-block-head-content">
                    <ul class="nk-block-tools g-3">
                        <li>
                            <a href="{{ route('admin.pricing.investment.schemes') }}" class="btn btn-primary d-none d-sm-inline-flex"><em class="icon ni ni-package-fill"></em><span>{{ __("Manage Scheme") }}</span></a>
                            <a href="{{ route('admin.pricing.investment.schemes') }}" class="btn btn-icon btn-primary d-inline-flex d-sm-none"><em class="icon ni ni-package-fill"></em></a>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <ul class="nk-nav nav nav-tabs mt-n3 mb-md-n3">
            <li class="nav-item">
                <a class="nav-link{{ ($listing=='active') ? ' active' : '' }}" href="{{ route('admin.pricing.investment.list', ['status' => 'active']) }}"><span>{{ __('Actived') }}</span></a>
            </li>
            <li class="nav-item">
                <a class="nav-link{{ ($listing=='pending') ? ' active' : '' }}" href="{{ route('admin.pricing.investment.list', ['status' => 'pending']) }}">
                    <span>{{ __('Pending') }} @if($pendingCount > 0) <span class="badge badge-primary">{{ $pendingCount }}</span> @endif </span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link{{ ($listing=='completed') ? ' active' : '' }}" href="{{ route('admin.pricing.investment.list', ['status' => 'completed']) }}">
                    <span>{{ __('Completed') }}</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link{{ ($listing=='all') ? ' active' : '' }}" href="{{ route('admin.pricing.investment.list') }}">
                    ...
                    <div class="card-inner p-0">
                        <div class="nk-tb-list nk-tb-ivx{{ user_meta('iv_invest_display') == 'compact' ? ' is-compact': '' }}">
                            ...
                            <div class="nk-tb-item">
                                ...
                                <div class="nk-tb-col tb-col-sm">
                                    <div class="user-card">
                                        ...
                                        <div class="user-name">
                                            ...
                                            <span>{{get_ref_code(data_get($plan, 'user_id'))}}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="nk-tb-col tb-col-lg">
                                    <span class="tb-sub">{{ data_get($plan, 'pvx') }}</span>
                                </div>
                                <div class="nk-tb-col">
                                    <span class="tb-sub tb-amount">{{ money(data_get($plan, 'total'), base_currency()) }}</span>
                                </div>
                                <div class="nk-tb-col tb-col-sm">
...
{{--                                    @endif--}}
                                ...
                                <div class="nk-tb-col nk-tb-col-action">
                                    <a href="{{ route('admin.pricing.investment.details', ['id' => the_hash($plan->id)]) }}" class="text-soft btn btn-sm btn-icon btn-trigger"><em class="icon ni ni-chevron-right"></em></a>
                                </div>
                            </div>
                            @endforeach
                        </div>
...
<script type="text/javascript">
    const updateSetting = "{{ route('admin.profile.update') }}",
        routes = {
            quick: "{{ route('admin.pricing.investment.plan.action') }}",
            approve: "{{ route('admin.pricing.investment.plan.approve') }}",
            reject: "{{ route('admin.pricing.investment.plan.cancel') }}"
        },
        msgs = {
            approved: {
