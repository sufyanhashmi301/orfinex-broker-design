...
<div class="p-6 pt-0">
        ...
        @foreach ($kycCredential as $key => $value)
            <li class="dark:text-slate-300 border-b border-slate-100 dark:border-slate-700 pb-3">
                ...
                @if (file_exists('assets/' . $value))
                    <div class="h-[260px] bg-no-repeat bg-contain bg-center bg-slate-100 mb-2"
    ...
    <form action="{{ route('admin.kyc.action.level3.now') }}" method="post">
        @csrf
        <input type="hidden" name="id" value="{{ $id }}">
        @php
            $kycComments = \App\Models\Comment::where('type', 'kyc')
                ->where('status', true)
                ->orderBy('title')
                ->get(['id', 'title', 'description']);
        @endphp
        <div class="input-area">
            <label class="form-label" for="kyc-comment-select-level3">
                <span class="shift-Away inline-flex items-center gap-1"
                    data-tippy-content="{{ __('Pick a template to prefill the message below') }}">
                    {{ __('Comments') }}
                    <iconify-icon icon="mdi:information-slab-circle-outline" class="text-[16px]"></iconify-icon>
                </span>
            </label>
            <select id="kyc-comment-select-level3" class="form-control select2 h-[42px]">
                <option value="">{{ __('Select a comment') }}</option>
                @forelse($kycComments as $comment)
                    <option value="{{ $comment->id }}" data-description='@json($comment->description)'>
            ...
            <p class="text-xs text-slate-400 mt-1">
                {{ __('Selecting a title will prefill the description. You can edit it further.') }}</p>
        </div>
        <div class="input-area">
            <label for="" class="form-label">
                <span class="shift-Away inline-flex items-center gap-1"
                    data-tippy-content="The detail message of the KYC">
                    {{ __('Detail Message') }}
                    <iconify-icon icon="mdi:information-slab-circle-outline" class="text-[16px]"></iconify-icon>
                </span>
            </label>
    ...
    <script>
        (function($) {
            ...
            $(document).ready(function() {
                if ($.fn.select2) {
                    $('#kyc-comment-select-level3').select2();
                }
                $('#kyc-comment-select-level3').on('change', function() {
