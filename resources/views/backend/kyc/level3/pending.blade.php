...
@endsection
@section('filters')
    <form id="filter-form" method="POST" action="{{ route('admin.kyc.export',['type' => 'level3']) }}">
        @csrf
...
@endsection
@section('style')
    @include('components.datatables.sort-icons', ['selector' => '#pending-kyc-dataTable'])
@endsection
@section('script')
    <script>
        (function ($) {
            ...
            .on('processing.dt', function (e, settings, processing) {
                ...
                autoWidth: false,
                ajax: {
                    url: "{{ route('admin.kyc.level3.pending') }}",
                    data: function (d) {
        ...
        $('body').on('click', '#action-kyc', function (e) {
            ...
            var id = $(this).data('id');
            var url = '{{ route("admin.kyc.level3.action",":id") }}';
