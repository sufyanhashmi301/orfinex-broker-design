            ...
            <div class="overflow-x-auto -mx-6">
                <div class="inline-block min-w-full align-middle">
                    <div class="overflow-hidden basicTable_wrapper">
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                @can('multi-ib-level-edit')
                                                    <button type="button" class="action-btn edit-level" data-bs-toggle="modal"
                                                        data-bs-target="#editLevelModal" data-id="{{ $level->id }}">
                                                        <iconify-icon icon="lucide:edit-3"></iconify-icon>
                                                    </button>
                                                @endcan
                                                @can('multi-ib-level-delete')
                                                    <button type="button" class="action-btn delete-level"
                                                        data-bs-toggle="modal" data-bs-target="#deleteLevelModal"
                                                        data-id="{{ $level->id }}" data-title="{{ $level->title }}">
                        ...
                        <div
                            class="flex justify-between items-center border-t border-slate-100 dark:border-slate-700 px-4 py-5 mt-auto">
                            <p class="text-sm text-gray-700">
                                Showing <span class="font-medium">{{ $levels->firstItem() }}</span> to <span
                                    class="font-medium">{{ $levels->lastItem() }}</span> of <span
                                    class="font-medium">{{ $levels->total() }}</span> results
                            </p>
                            {{ $levels->links() }}
                        </div>
                    </div>
