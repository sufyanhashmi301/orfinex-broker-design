        ...
        <div class="flex flex-col sm:flex-row justify-between flex-wrap sm:items-center gap-3">
            <div class="flex-1 w-full flex flex-col sm:flex-row sm:gap-3 gap-2">
                <div class="flex-1 input-area relative">
                    <input type="text" name="global_search" id="global_search" class="form-control h-full" placeholder="Search by Form Name">
                </div>
                <div class="flex-1 input-area relative">
            ...
            <div class="flex sm:space-x-3 space-x-2 sm:justify-end items-center">
                ...
                <div class="input-area relative">
                    <a href="{{ route('admin.payment-deposit-form.create') }}" class="btn btn-sm btn-primary inline-flex items-center justify-center min-w-max">
                        ...
                        <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700">
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-none">
                                                <div class="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
                                                    <iconify-icon icon="lucide:file-text" class="text-slate-500 dark:text-slate-400"></iconify-icon>
                                                </div>
                                            </div>
                                            <div class="ltr:ml-3 rtl:mr-3">
                                                <span class="text-slate-600 dark:text-slate-300 text-sm font-medium">{{ $question->name }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="table-td">
                                    ...
                                    <td class="table-td">
                                        <span class="text-slate-500 dark:text-slate-400">{{ is_array($question->fields) ? count($question->fields) : 0 }} {{ __('fields') }}</span>
                                    </td>
                                    <td class="table-td">
                                        <span class="text-slate-500 dark:text-slate-400">{{ $question->created_at->format('M d, Y') }}</span>
                                        <br>
                                        <span class="text-xs text-slate-400">{{ $question->created_at->format('h:i A') }}</span>
                                    </td>
                                    <td class="table-td">
                                        <div class="flex space-x-3 rtl:space-x-reverse">
                                            <a href="{{ route('admin.payment-deposit-form.edit', $question->id) }}" class="action-btn toolTip onTop" data-tippy-theme="dark" data-tippy-content="Edit" title="Edit">
                                                <iconify-icon icon="lucide:edit-3"></iconify-icon>
                                            </a>
                                            <form action="{{ route('admin.payment-deposit-form.destroy', $question->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="action-btn text-danger-500 toolTip onTop" data-tippy-theme="dark" data-tippy-content="Delete" title="Delete" 
                                ...
                                <tr>
                                    <td colspan="5" class="table-td text-center py-12">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h4 class="text-slate-500 dark:text-slate-400 font-medium mb-2">{{ __('No Forms Found') }}</h4>
                                            <p class="text-slate-400 text-sm mb-4">{{ __('Create your first payment deposit form to get started.') }}</p>
                                            <a href="{{ route('admin.payment-deposit-form.create') }}" class="btn btn-sm btn-primary">
