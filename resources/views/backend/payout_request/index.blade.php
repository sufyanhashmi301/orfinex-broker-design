    ...
    <div class="innerMenu flex justify-between flex-wrap items-center gap-5 mb-5">
        <ul class="nav nav-tabs custom-tabs inline-flex items-center overflow-hidden rounded list-none border-0 pl-0" id="tabs-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <a href="?status=all" class="btn btn-sm inline-flex justify-center btn-outline-primary {{ request('status') === 'all' ? 'active' : '' }}" aria-controls="tabs-demoAccounts" aria-selected="false">{{ __('All Payout Requests') }}</a>
            </li>
            <li class="nav-item" role="presentation">
                <a href="?status={{ \App\Enums\PayoutRequestStatus::PENDING }}" class="btn btn-sm inline-flex justify-center btn-outline-primary {{ request('status') === \App\Enums\PayoutRequestStatus::PENDING ? 'active' : '' }}" aria-controls="tabs-demoAccounts" aria-selected="false">{{ __('Pending Payouts') }}</a>
            </li>
            <li class="nav-item" role="presentation">
                <a href="?status={{ \App\Enums\PayoutRequestStatus::APPROVED }}" class="btn btn-sm inline-flex justify-center btn-outline-primary {{  request('status') === \App\Enums\PayoutRequestStatus::APPROVED ? 'active' : '' }}" aria-controls="tabs-archivedAccounts" aria-selected="false">{{ __('Approved Payouts') }}</a>
            </li>
            <li class="nav-item" role="presentation">
              <a href="?status={{ \App\Enums\PayoutRequestStatus::DECLINE }}" class="btn btn-sm inline-flex justify-center btn-outline-primary {{  request('status') === \App\Enums\PayoutRequestStatus::DECLINE ? 'active' : '' }}" aria-controls="tabs-archivedAccounts" aria-selected="false">{{ __('Declined Payouts') }}</a>
          </li>
