@php
    ...
    $checkRecord = \App\Models\PlatformGroup::where('group',$Group)->first();
    $checked = in_array($Group, $existingGroups) && $checkRecord->status==1 ? 'checked' : '';
@endphp
<div class="form-switch">
    <label class="relative !inline-flex h-6 w-[46px] items-center rounded-full transition-all duration-150 cursor-pointer toggle-checkbox">
        <input
            ...
            onchange="insertRecord({{ $Group_ID }}, $(this).data('previous-state'))"
            {{ $checked }}>
