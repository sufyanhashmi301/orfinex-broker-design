        ...
        <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
            <div class="modal-body popup-body">
                ...
                <div class="popup-body-text p-6 pt-0">
                    <form action="{{ route('admin.referral.target-store') }}" method="post" class="space-y-5">
                        @csrf
                        <div class="input-area">
                            <label for="" class="form-label">{{ __('Target Name:') }}</label>
                            <input type="text" name="name" class="form-control mb-0" placeholder="Target #1" required=""/>
                        </div>
