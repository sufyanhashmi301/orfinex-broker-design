<form action="{{ route('admin.risk-profile-tag.update',$riskProfileTag->id) }}" method="post">
    @method('PUT')
    @csrf
    <div class="space-y-5">
        ...
        <div class="input-area">
            ...
            <textarea name="desc" class="form-control mb-0" rows="6" placeholder="Details">{{ old('name',$riskProfileTag->desc) }}</textarea>
        </div>
        <div class="input-area">
            <div class="flex items-center space-x-7 flex-wrap">
                ...
                <div class="form-switch ps-0" style="line-height: 0;">
                    ...
                    <label class="deposit-status relative inline-flex h-6 w-[46px] items-center rounded-full transition-all duration-150 cursor-pointer toggle-switch">
                        <input type="checkbox" name="status" value="1" class="sr-only peer" @if($riskProfileTag->status) checked @endif>
