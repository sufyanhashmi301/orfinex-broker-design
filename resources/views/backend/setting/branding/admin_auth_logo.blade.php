...
@section('website-content')
    <?php
        ...
        $currentLogo = setting('admin_auth_logo_image', 'admin_auth_logo');
                ...
                <form action="{{ route('admin.settings.update') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="relative flex justify-between items-center bg-white dark:bg-slate-800 rounded shadow-base px-6 py-5">
                        ...
                        <div class="form-switch ps-0" style="line-height: 0;">
                            <input type="hidden" value="0" name="admin_auth_logo_status">
                            <label class="relative inline-flex h-6 w-[46px] items-center rounded-full transition-all duration-150 cursor-pointer toggle-switch">
                                <input type="checkbox" name="admin_auth_logo_status" value="1" class="sr-only peer" 
                                    @if(setting('admin_auth_logo_status', 'admin_auth_logo')) checked @endif>
                            ...
                            <div class="input-area">
                                ...
                                <div class="wrap-custom-file">
                                    <input
                                        ...
                                        id="admin_auth_logo-image"
                                        accept=".jpg,.jpeg,.png,.svg"
                                    />
                                    <label for="admin_auth_logo-image"
                            ...
                            <div class="input-area">
                                ...
                                <div class="mt-3 p-3 bg-slate-100 dark:bg-slate-700 rounded-md">
                                    <p class="text-sm text-slate-600 dark:text-slate-300">
                                        ...
                                        {{ __('When enabled, this logo shows on the admin login page. When disabled, no logo is shown on the admin login page.') }}
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const logoInput = document.getElementById('admin_auth_logo-image');
            if (logoInput) {
                logoInput.addEventListener('change', function(e) {
                    ...
                    if (file) {
                        ...
                        reader.onload = function(event) {
                            const label = document.querySelector('label[for="admin_auth_logo-image"]');
