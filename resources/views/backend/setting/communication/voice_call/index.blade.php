...
<div class="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
  @foreach ($voice_call_methods as $method)
      <div class="card border hover:shadow-lg">
          <div class="card-header items-center noborder !p-4">
              ...
              <div style="position: relative; top: -5px">
                @if ($method->method == 'twilio')
                  <button type="button" style="display: inline-block" class="action-btn cursor-pointer dark:text-slate-300" data-bs-toggle="modal" data-bs-target="#configureTwilio">
          ...
          <div>
              ...
              @if ($method->status)
                <a href="{{ route('admin.settings.voice_call.toggle_method', ['id' => $method->id, 'action' => 'disable']) }}" type="button" class="btn btn-primary btn-sm float-right"  style="margin-right: 10px; margin-bottom: 10px; min-width: initial">Deactivate</a>
              @else
                <a href="{{ route('admin.settings.voice_call.toggle_method', ['id' => $method->id, 'action' => 'active']) }}" type="button" class="btn btn-primary btn-sm float-right {{ empty($method->details) ? 'disabled' : '' }}"  style="margin-right: 10px; margin-bottom: 10px; min-width: initial">Activate</a>
              @endif
  ...
  @include('backend.setting.communication.voice_call.includes.__configure_twilio_modal', ['method' => $voice_call_methods->where('method', 'twilio')->first()])
