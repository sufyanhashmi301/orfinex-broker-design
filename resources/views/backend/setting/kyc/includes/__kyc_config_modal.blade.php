        ...
        <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
            ...
            <div class="modal-body p-6 pt-0">
                <form action="{{ route('admin.kyc_notice.update') }}" method="post" class="space-y-5">
                    ...
                    <div class="site-input-area relative">
                        <label for="" class="form-label">KYC Badge Visibility</label>
                        <select name="kyc_badge_visibility" class="form-control capitalize" id="" >
                          <option value="show" {{ setting('kyc_badge_visibility', 'defaults') == 'show' ? 'selected' : '' }} >Show</option>
                          <option value="hide" {{ setting('kyc_badge_visibility', 'defaults') == 'hide' ? 'selected' : '' }} >Hide</option>
                          <option value="show_verified" {{ setting('kyc_badge_visibility', 'defaults') == 'show_verified' ? 'selected' : '' }} >Show when Verified</option>
                          <option value="show_unverified" {{ setting('kyc_badge_visibility', 'defaults') == 'show_unverified' ? 'selected' : '' }} >Show when Not Verified</option>
                        </select>
                      </div>  
