<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
     id="deleteSymbolGroup"
     tabindex="-1"
     aria-labelledby="deleteSymbolGroup"
     aria-hidden="true">
            ...
            <div class="modal-body p-6 py-10 text-center">
                <div class="space-y-3">
                    ...
                    <p>
                        ...
                        <strong class="name"></strong> {{ __('Symbol Group?') }}
                    </p>
                    <div id="attached-rules" class="hidden">
                        <div class="text-left max-h-60 overflow-y-auto">
                            <h5 class="font-medium mb-2">{{ __('Attached Rebate Rules') }}:</h5>
                            <ul class="rules-list"></ul>
                        </div>
                        <div class="mt-4 text-red-500">
                            {{ __('Please detach these rebate rules first before deleting the group.') }}
                        </div>
                    </div>
                    <div id="no-rules" class="hidden">
                        <p class="text-green-500">{{ __('No rebate rules are attached to this group.') }}</p>
                    </div>
                </div>
                <form method="POST" id="symbolGroupDeleteForm">
                    @csrf
                    @method('DELETE')
