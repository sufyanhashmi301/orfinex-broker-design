<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="systemTagModal" tabindex="-1" aria-labelledby="systemTagModal" aria-hidden="true">
    <div class="modal-dialog top-1/2 !-translate-y-1/2 relative w-auto pointer-events-none">
        ...
        rounded-md outline-none text-current">
            <div class="modal-body popup-body">
                ...
                <div class="p-6 pt-0">
                    <form action="{{ route('admin.system-tag.store') }}" method="post" class="space-y-4">
                        @csrf
                        <div class="input-area">
                            <label class="form-label" for="">{{ __('Name:') }}</label>
                            <input type="text" name="name" value="{{ old('name') }}" class="form-control" placeholder="System Tag Name" required/>
                        </div>
                        <div class="input-area">
                            <label for="" class="form-label">{{ __('Details (Optional)') }}</label>
                            <textarea name="desc" class="form-control mb-0" rows="6" placeholder="Details about the system tag"></textarea>
                        </div>
                        <div class="input-area">
