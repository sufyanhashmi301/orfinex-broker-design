<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
    id="addSubBal" tabindex="-1" aria-labelledby="addSubBalModalLabel" aria-hidden="true">
                        ...
                        <div class="input-area">
                            <label class="form-label" for="">
                                <span class="shift-Away inline-flex items-center gap-1"
                                    data-tippy-content="Select the account to add or subtract balance from">
                                    {{ __('Select Account') }}
                            ...
                            <select class="select2 form-control w-100" name="target_id" id="tradingAccount_balance">
                                ...
                                @foreach (get_all_wallets($user->id) as $wallet)
                                    <option value="{{ $wallet->wallet_id }}" data-type="wallet">
                                        {{ $wallet->wallet_id }} - {{ w2n($wallet->balance) }} ({{ $wallet->amount }}
                                        {{ $currency }})
                                    </option>
                                @endforeach
                            </select>
