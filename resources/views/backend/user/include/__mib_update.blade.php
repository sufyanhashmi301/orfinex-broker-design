<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
    id="updateMIBModal"
    tabindex="-1"
    aria-labelledby="updateMIBModal"
    aria-hidden="true"
>
        ...
        <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white dark:bg-dark bg-clip-padding rounded-md outline-none text-current">
            <div class="flex items-center justify-between p-5">
                <h3 class="text-xl font-medium dark:text-white capitalize">
                    {{ __('Update Multi IB To ') }} <span id="name">{{ $user->full_name ?? ''}}</span>
                </h3>
            ...
            <div class="modal-body p-6 pt-0">
                <form action="{{ route('admin.ib.multi.update') }}" method="post" class="space-y-5">
                    @csrf

                    <input type="hidden" name="user_id" value="{{$user->id}}">

                    <div class="input-area">
                        <label for="" class="form-label">{{ __('Multi IB Login:') }}</label>
                        <input
                            type="text"
                            name="multi_ib_login"
