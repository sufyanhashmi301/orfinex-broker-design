<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
    id="addReferralModal" tabindex="-1" aria-labelledby="addReferralModal" aria-hidden="true">
    <div class="modal-dialog top-1/2 !-translate-y-1/2 relative w-auto pointer-events-none">
        <div
            class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
            <div class="flex items-center justify-between p-5">
                <h3 class="text-xl font-medium dark:text-white capitalize">
                    {{ __('Add Referral Under ') }} <span id="referral-add-name">{{ $user->full_name ?? '' }}</span>
                </h3>
                <button type="button"
            ...
            <div class="modal-body p-6 pt-0">
                {{-- {{ route('admin.referral.direct.add') }} --}}
                <form action="" method="post" class="space-y-5">
                    ...
                    <div class="input-area">
                        ...
                        <div class="relative ">
                            <select name="user_id" id="countrySelect"
                                class="select2 form-control py-2 h-[48px] w-full mt-2">
