    ...
    <div class="flex flex-col gap-5 mb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-start w-full gap-3 sm:justify-end">
            <div class="inline-flex w-fit items-center gap-0.5 rounded-lg bg-gray-100 p-0.5 dark:bg-gray-900">
                <button @click="activeTab = 'socialMediaMaterial'" :class="activeTab === 'socialMediaMaterial' ? 'shadow-theme-xs text-gray-900 dark:text-white bg-white dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400'" class="px-3 py-2 font-medium rounded-md text-theme-sm hover:text-gray-900 dark:hover:text-white transition-all duration-200">
                    {{ __('Social media material') }}
                </button>
                <button @click="activeTab = 'websiteBanners'" :class="activeTab === 'websiteBanners' ? 'shadow-theme-xs text-gray-900 dark:text-white bg-white dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400'" class="px-3 py-2 font-medium rounded-md text-theme-sm hover:text-gray-900 dark:hover:text-white transition-all duration-200">
                    {{ __('Website banners') }}
                </button>
            </div>
            <div class="relative w-fit">
                <select class="h-10 rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs focus:outline-hidden focus:ring-0 focus-visible:outline-hidden dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400" name="language" id="language">
