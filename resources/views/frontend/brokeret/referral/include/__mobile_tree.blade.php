<div x-data="{ open: false }" class="treeview__level relative z-[1]" data-level="{{ $level }}">
    <div class="flex items-center relative z-[1]" style="padding-left: 20px;">
        ...
        <div class="text-start inline-flex items-center rounded border border-slate-100 dark:border-slate-700 gap-3 p-2 py-1 pl-8">
            @if($me)
                <h4 class="text-sm font-medium text-slate-600 dark:text-white whitespace-nowrap py-2">
            ...
            @if($levelUser->referrals->count() > 0)
                <button type="button"
                    @click="open = !open"
                    class="inline-flex h-6 w-6 items-center justify-center rounded-full border-[0.5px] border-gray-200 bg-gray-50 text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 ml-2 horizontal-toggle-btn">
