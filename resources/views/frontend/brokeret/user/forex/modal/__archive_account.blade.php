<div
    x-show="$store.modals.isOpen('archiveAccount')"
    x-transition
    ...
    <div @click.away="$store.modals.close()" class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
        <div class="text-center">
            ...
            <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                {{ __('If you archive this account you can\'t update info/use') }}
            </p>
            <form @submit.prevent="$store.modals.archiveAccount($refs)">
                <input type="hidden" x-ref="login" :value="$store.modals.data.login">
                
                <div class="flex items-center justify-center w-full gap-3 mt-7">
                    <button type="submit" x-ref="submitBtn" class="inline-flex items-center gap-2 px-5 py-3.5 text-sm font-medium text-white rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600 sm:w-auto">
