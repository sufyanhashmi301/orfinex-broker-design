<div
    x-show="$store.modals.isOpen('changeLeverage')"
    x-transition
    ...
    <div @click.away="$store.modals.close()" class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="block text-gray-500 dark:text-gray-400">
                    {{ __('Account:') }} <span x-text="$store.modals.data.login">
                </p>
            </div>
            <!-- Close Button -->
        ...
        <div>
            ...
            <div x-show="$store.modals.loading" class="text-center py-6">
                <svg class="mx-auto h-8 w-8 animate-spin text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
                </svg>
                <p class="mt-2 text-gray-600 dark:text-gray-400">{{ __('Loading...') }}</p>
            </div>
            
            <div x-html="$store.modals.html" x-show="!$store.modals.loading"></div>
        </div>
    </div>
</div>
