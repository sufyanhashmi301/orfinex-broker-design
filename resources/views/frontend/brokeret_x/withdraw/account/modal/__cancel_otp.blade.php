    ...
    <div @click.stop class="relative w-full max-w-xl rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
        <div class="text-center">
            ...
            <div class="flex items-center justify-center w-full gap-3 mt-7">
                <x-frontend::forms.button type="button" variant="primary" size="md" icon="check" icon-position="left" @click="confirmCancel">
                    {{ __('Confirm') }}
                </x-frontend::forms.button>
                <x-frontend::forms.button type="button" variant="secondary" size="md" icon="x" icon-position="left" @click="isCancelModalOpen = false">
                    {{ __('Close') }}
                </x-frontend::forms.button>
            </div>
        </div>
