<div
    x-show="$store.modals.isOpen('deleteAccount')"
    x-transition
    ...
    <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
        <div class="text-center">
            ...
            <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                {{ __('you want to delete this account!.') }}
            </p>
            <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                {{ __('If you delete this account you can\'t update info/use') }}
            </p>
            <form @submit.prevent="$store.modals.deleteAccount($refs)">
                <input type="hidden" x-ref="id" :value="$store.modals.data.id">
                
                <div class="flex items-center justify-center w-full gap-3 mt-7">
                    <x-frontend::forms.button type="submit" x-ref="submitBtn" variant="primary" size="md" icon="trash" icon-position="left">
                        {{ __('Delete Account') }}
