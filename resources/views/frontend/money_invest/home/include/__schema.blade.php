                ...
                <div class="single-investment-plan" data-aos="fade-up" data-aos-duration="1500">
                    ...
                    <div class="content">
                        ...
                        <ul>
                            ...
                            <li><i icon-name="check-check"></i>{{ __('Return Type') }}<span>{{ __(ucwords($schema->return_type)) }}</span></li>
                            <li><i icon-name="check-check"></i>{{ __('Total Periods') }}<span>{{ ($schema->return_type == 'period' ? $schema->number_of_period.' ' : __('Unlimited').' ' ).($schema->number_of_period == 1 ? __('Time') : __('Times') )  }}</span></li>
                            <li><i icon-name="check-check"></i>{{ __('Cancellation') }}<span>@if($schema->schema_cancel) {{ __('In').' '. $schema->expiry_minute .' '. 'Minute' }} @else   {{ __('No') }}@endif</span></li>
                            <li><i icon-name="check-check"></i>{{ __('Total Revenue') }}<span> {{ $schema->total_revenue }}</span></li>
                        </ul>
                    ...
                    <a href="{{route('user.schema.preview',$schema->id)}}" class="investment-btn w-100 centered">{{ __('Invest Now') }}<i icon-name="move-right"></i></a>
                </div>
            </div>
            @endforeach
        </div>
