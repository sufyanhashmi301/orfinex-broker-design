...
@section('deposit_content')
    <div class="progress-steps-form mb-6">
        <form action="{{ route('user.deposit.redeem.now') }}" method="post" enctype="multipart/form-data" id="voucherForm">
            @csrf
                    ...
                    <div class="card">
                        <div class="card-body p-6 space-y-5">
                            <div class="input-area relative">
                                ...
                                <div class="input-group select2-lg">
                                    <select id="tradingAccount" name="target_id"
                                        ...
                                        @endforeach
                                        {{-- mail wallet --}}
                                        @include('frontend::wallet.include.__specific-wallet-dropdown', [
                                            'target_id_name' => 'target_id',
                                            'wallet_type' => \App\Enums\AccountBalanceType::MAIN,
                                        ])
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="input-area relative">
                                ...
                                <div class="relative">
                                    <input type="text" name="amount" class="form-control !text-lg" id="voucher-amount"
                                        readonly>
                                    <span
                            ...
                            <table class="table w-full border-collapse table-fixed dark:border-slate-700 dark:border">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="charge2 dark:text-slate-100"></td>
                                    </tr>
                                    <tr>
                                        <td
                                            class="text-slate-900 dark:text-slate-100 text-sm font-normal ltr:text-left ltr:last:text-right rtl:text-right rtl:last:text-left px-6 py-4">
                                        ...
                                        <td class="total dark:text-slate-100"></td>
                                    </tr>
                                    <tr class="conversion">
                                        <td
                                            class="text-slate-900 dark:text-slate-100 text-sm font-normal ltr:text-left ltr:last:text-right rtl:text-right rtl:last:text-left px-6 py-4">
                                        ...
                                        <td class="conversion-rate dark:text-slate-100"></td>
                                    </tr>
                                    <tr class="conversion">
                                        <td
                                            class="text-slate-900 dark:text-slate-100 text-sm font-normal ltr:text-left ltr:last:text-right rtl:text-right rtl:last:text-left px-6 py-4">
                                        ...
                                        <td class="pay-amount dark:text-slate-100"></td>
                                    </tr>
                                    <tr>
                                        <td
                                            class="text-slate-900 dark:text-slate-100 text-sm font-normal ltr:text-left ltr:last:text-right rtl:text-right rtl:last:text-left px-6 py-4">
                                        ...
                                        <td class="expires-at dark:text-slate-100"></td>
                                    </tr>
                                </tbody>
                            </table>
                            <div class="buttons border-t border-slate-100 dark:border-slate-700 mt-4 pt-4">
                                <button type="submit" class="btn w-full inline-flex justify-center btn-primary proceed-btn space-x-2" data-loading-text="Processing...">
    ...
    <div
        ...
        <div class="accordion-item">
            <h2 class="accordion-header text-lg" id="flush-headingStaySafeOnline">
                <button class="flex items-center w-full accordion-button collapsed" type="button" data-bs-toggle="collapse"
                    data-bs-target="#flush-staySafeOnline" aria-expanded="false" aria-controls="flush-staySafeOnline">
                    <iconify-icon class="text-xl mr-2" icon="lucide:info"></iconify-icon>
                    {{ __('Stay safe online') }}
                    <iconify-icon class="chevron-icon transition-transform duration-200 ease-in-out ml-auto"
                        icon="lucide:chevron-down"></iconify-icon>
                </button>
            </h2>
            <div id="flush-staySafeOnline" class="accordion-collapse collapse" aria-labelledby="flush-headingStaySafeOnline"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function updateVoucherCalculation(amount) {
                ...
                var charge = globalData.charge_type === 'percentage' ? calPercentage(amount, globalData.charge) :
                    globalData.charge
                ...
                $('.pay-amount').text(parseFloat((total * globalData.rate).toFixed(4)).toString() + ' ' + globalData
                    .currency)
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('.invalid-code').addClass('hidden').text('');
                            $('.proceed-btn').removeAttr('disabled').removeClass(
                                'cursor-not-allowed');
                        } else {
                            ...
                            $('.proceed-btn').attr('disabled', true).addClass(
                                'cursor-not-allowed');
                        }
                    }
                })
        ...
        $(document).on('submit', '#voucherForm', function () {
            const $form = $(this);
