        ...
        <div class="flex justify-between items-center h-full">
            ...
            <div class="flex items-center md:space-x-4 space-x-2 mr-2 xl:space-x-0 rtl:space-x-reverse vertical-box">
                <a href="{{ route('user.dashboard') }}" class="mobile-logo xl:hidden inline-block active">
                    <img src="{{ getFilteredPath(setting('site_mobile_logo', 'theme'), 'fallback/branding/mobile-admin-logo.png') }}"
                        class="black_logo h-8" alt="logo">
                    <img src="{{ getFilteredPath(setting('site_mobile_logo_light', 'theme'), 'fallback/branding/mobile-admin-logo.png') }}"
                        class="white_logo h-8" alt="logo">
                </a>
                <button
                ...
                <div class="overflow-x-auto">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden">
                            <ul
                                class="nav nav-tabs custom-tabs inline-flex items-center overflow-hidden rounded list-none border-0 pl-0">
                                <li class="nav-item">
                                    <a href="{{ route('user.follower_access') }}"
                                        class="btn btn-sm !text-nowrap inline-flex justify-center btn-outline-primary loaderBtn {{ isActive('user.follower_access') }}">
                                        {{ __('Follower Access') }}
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('user.provider_access') }}"
                                        class="btn btn-sm !text-nowrap inline-flex justify-center btn-outline-primary loaderBtn {{ isActive('user.provider_access') }}">
                                        {{ __('Provider Access') }}
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('user.ratings') }}"
                                        class="btn btn-sm !text-nowrap inline-flex justify-center btn-outline-primary loaderBtn {{ isActive('user.ratings') }}">
                                        {{ __('Ratings') }}
                                    </a>
                                </li>
                            </ul>
                ...
                <div class="relative">
                    <div class="flex items-center text-left">
                        ...
                        @if ($user->kyc >= \App\Enums\KYCStatus::Level2->value)
                            <img src="https://cdn.brokeret.com/crm-assets/admin/kyc/verified.svg"
                                class="inline-flex ml-2 mt-1" alt="" style="height: 14px;">
                        @else
                            <img src="https://cdn.brokeret.com/crm-assets/admin/kyc/unverified.svg"
                                class="inline-flex ml-2 mt-1" alt="" style="height: 14px;">
                        @endif
                    </div>
                </div>
            ...
            <div class="nav-tools flex items-center md:space-x-4 space-x-3 rtl:space-x-reverse leading-0 ml-auto">
                ...
                @if (session('impersonating_admin_id'))
                    <div class="relative">
                        <a href="{{ route('user.return-to-admin') }}"
                            class="btn btn-sm btn-danger inline-flex items-center justify-center">
                ...
                <div class="relative">
                    <form action="{{ route('user.setting.preference.theme') }}" method="POST" class="mb-0">
                        ...
                        @if (auth()->user()->user_theme == 'light')
                            ...
                            <button type="submit"
                                class="lg:h-[32px] lg:w-[32px] dark:text-slate-900 text-white cursor-pointer rounded-lg text-[20px] flex flex-col items-center justify-center">
                                <iconify-icon class="header-text-color text-xl" id="moonIcon"
                        ...
                        @elseif(auth()->user()->user_theme == 'dark')
                            ...
                            <button type="submit"
                                class="lg:h-[32px] lg:w-[32px] dark:text-slate-900 text-white cursor-pointer rounded-lg text-[20px] flex flex-col items-center justify-center">
                                <iconify-icon class="header-text-color text-xl" id="sunIcon"
                ...
                <div class="relative hidden md:block">
                    <button
                        class="lg:h-[32px] lg:w-[32px] cursor-pointer rounded-lg text-[20px] flex flex-col items-center justify-center"
                        type="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <iconify-icon class="header-text-color text-xl" icon="lucide:globe"></iconify-icon>
                    </button>
                    <!-- Dropdown menu -->
                    <div
                        class="dropdown-menu z-10 hidden bg-white divide-y divide-slate-100 shadow w-44 border dark:border-slate-700 !top-[10px] rounded-md overflow-hidden">
                        <ul class="py-1 text-sm text-slate-800 dark:text-slate-200">
                            @foreach (\App\Models\Language::where('status', true)->get() as $lang)
                                <li>
                                    <a href="{{ route('language-update', ['name' => $lang->locale]) }}"
                                        class="loaderBtn block px-4 py-2 hover:bg-slate-100 dark:hover:bg-dark dark:hover:text-white font-inter text-sm text-slate-600 dark:text-white font-normal">
                ...
                @auth
                    ...
                    <div class="relative md:block hidden user-notifications{{ $userId }}">
                        @include('global.__notification_data', [
                ...
                <div>
                    <button
                        class="item notification-dot lg:h-[32px] lg:w-[32px] dark:text-slate-900 text-white cursor-pointer rounded-lg text-[20px] hidden md:flex flex-col items-center justify-center"
                ...
                <div class="md:block hidden w-full">
                    <button
                        ...
                        <div class="lg:h-8 lg:w-8 h-7 w-7 rounded-full flex-1 border-2 border-primary">
                            <img src="{{ getFilteredPath(auth()->user()->avatar, 'fallback/user.png') }}"
                                alt="user" class="block w-full h-full object-cover rounded-full">
                        </div>
                    </button>
                    <!-- Dropdown menu -->
                    <div
                        class="dropdown-menu z-10 hidden bg-white divide-y divide-slate-100 shadow min-w-max border dark:border-slate-700 !top-[10px] rounded-md overflow-hidden">
                        <ul class="py-1 text-sm text-slate-800 dark:text-slate-200">
                            <li>
                            ...
                            <li>
                                <a href="{{ route('user.change.password') }}"
                                    class="loaderBtn block px-4 py-2 hover:bg-slate-100 dark:hover:bg-dark dark:hover:text-white font-inter text-sm text-slate-600 dark:text-white font-normal">
                                    <iconify-icon icon="lucide:lock-keyhole"
                                        class="relative top-[2px] text-lg ltr:mr-1 rtl:ml-1"></iconify-icon>
                            ...
                            <li>
                                <form method="POST" action="{{ route('logout') }}" id="logout-form">
                                    ...
                                    <a href="{{ url('logout') }}"
                                        ...
                                        class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-dark dark:hover:text-white font-inter text-sm text-slate-600 dark:text-white font-normal">
                    ...
                    <div
                        ...
                        <ul class="py-1 text-sm text-slate-800 dark:text-slate-200">
                            ...
                            @if (setting('is_webterminal', 'global'))
                                <li>
                                    <a href="{{ route('webterminal') }}"
                                        class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-dark dark:hover:text-white font-inter text-sm text-slate-600 dark:text-white font-normal"
                                        target="_blank">
                                        <iconify-icon icon="ic:outline-candlestick-chart"
                                            class="relative top-[2px] text-lg ltr:mr-1 rtl:ml-1"></iconify-icon>
                            ...
                            @endif
                            <li>
                                <a href="{{ setting('company_website', 'common_settings') }}"
                                    class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-dark dark:hover:text-white font-inter text-sm text-slate-600 dark:text-white font-normal"
                                    target="_blank">
                                    <iconify-icon icon="tabler:world-www"
                                        class="relative top-[2px] text-lg ltr:mr-1 rtl:ml-1"></iconify-icon>
                            ...
                            @if (auth()->user()->ib_status == \App\Enums\IBStatus::APPROVED)
                                <li>
                                    <a href="{{ route('user.multi-level.ib.dashboard') }}"
                                        class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-dark dark:hover:text-white font-inter text-sm text-slate-600 dark:text-white font-normal"
                                        target="_blank">
