    ...
    <a href="{{ route('home') }}" class="loaderBtn items-center md:flex hidden">
        <span class="black_logo">
            @if (isDarkColor(setting('header_bg', 'light_colors')))
                <img src="{{ getFilteredPath(setting('site_logo_light', 'global'), 'fallback/branding/desktop-logo.png') }}"
                    class="max-w-[160px]" alt="{{ __('Light Logo') }}" />
            @else
                <img src="{{ getFilteredPath(setting('site_logo', 'global'), 'fallback/branding/desktop-logo.png') }}"
                    class="max-w-[160px]" alt="{{ __('Dark Logo') }}" />
            @endif
        </span>
            ...
            <div class="flex-none">
                <div class="w-8 h-8 rounded-[100%] ltr:mr-2 rtl:ml-2">
                    <img src="{{ getFilteredPath(auth()->user()->avatar, 'fallback/user.png') }}"
                        alt="{{ __('User Profile') }}" class="w-full h-full rounded-[100%] object-cover">
                </div>
            </div>
    ...
    @endif
        ...
        <li>
            <a href="{{ route('user.deposit.methods') }}"
                class="navItem loaderBtn @if (Route::currentRouteName() != 'user.deposit.log') {{ isActive('user.deposit*') }} @endif">
                <span class="flex items-center">
        ...
        <li>
            <a href="{{ route('user.withdraw.view') }}"
                class="navItem loaderBtn @if (Route::currentRouteName() != 'user.withdraw.log') {{ isActive('user.withdraw*') }} @endif">
                <span class="flex items-center">
        ...
        @if (setting('is_copy_trading', 'copy_trading'))
            <li>
                <a href="{{ route('user.follower_access') }}"
                    class="navItem loaderBtn {{ isActive('user.follower_access') }}">
                    <span class="flex items-center">
        ...
        @endif
        <li>
            <a href="{{ route('user.history.transactions') }}"
                class="navItem loaderBtn {{ isActive('user.history*') }}">
                <span class="flex items-center">
        ...
        @if (auth()->user()->ib_status == \App\Enums\IBStatus::APPROVED || isset(auth()->user()->ref_id))
            <li>
                <a href="{{ route('user.multi-level.ib.dashboard') }}"
                    class="navItem loaderBtn {{ isActive('user.referral') }}">
                    <span class="flex items-center">
        ...
        <li class="block md:hidden">
            <form method="POST" action="{{ route('logout') }}" id="logout-form" class="mt-5 mb-3">
                ...
                <a href="{{ url('logout') }}"
                    ...
                    class="btn btn-base btn-sm block">
