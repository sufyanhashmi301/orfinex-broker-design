@php
    $successPage = getSuccessPage('transfer');
    $title = $successPage->title ?? __('Your transaction was processed successfully!');
    $subtitle = $successPage->subtitle ?? __('Transfer Successful!');
    $message = $successPage->message ?? __('Your transfer request has been processed successfully. You will receive an email with the details of your transfer.');
    ...
    $buttonText = $successPage->button_text ?? __('Go to Transfer');
    $buttonLink = $successPage->button_link ?? route('user.transfer');
