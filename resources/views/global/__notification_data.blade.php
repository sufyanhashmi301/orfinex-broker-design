...
<div class="dropdown-menu dropdown-menu-end notification-pop dropdown-menu z-10 hidden bg-white shadow w-[335px] dark:bg-slate-800 border dark:border-slate-700 !top-[23px] rounded-md overflow-hidden lrt:origin-top-right rtl:origin-top-left">
    <div class="d-flex align-items-center justify-content-between flex items-center justify-between py-4 px-4">
        ...
        <a class="text-xs font-Inter font-normal underline text-slate-500 dark:text-white" href="{{ route($notifications->first()->for.'.notification.all') }}">
            {{ __('See All') }}
        </a>
    </div>
    <div class="all-noti divide-y divide-slate-100 dark:divide-slate-900">
        @foreach($notifications as $notification)
            <div class="single-noti text-slate-600 dark:text-slate-300 block w-full px-4 py-2 text-sm">
                <a href="{{ route($notification->for.'.read-notification', $notification->id) }}" class="flex ltr:text-left rtl:text-right" @class(['read' => $notification->read ])>
