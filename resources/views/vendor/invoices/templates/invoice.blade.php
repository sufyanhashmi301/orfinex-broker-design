    ...
    <body>
                ...
                <tr>
                    ...
                    <td class="px-0">
                        @if($invoice->buyer->name)
                            <p class="buyer-name" style="text-transform: capitalize">
        ...
        <table class="table table-items">
            ...
            <tbody>
                ...
                @endif
                @if($invoice->getCustomData()['has_addons']))

                    @foreach ($invoice->getCustomData()['addons'] as $addon)
                        <tr>
                            <td colspan="{{ $invoice->table_columns - 2 }}" class="border-0"></td>
                            <td class="text-right pl-0">{{ $addon['name'] }} Addon</td>
                            <td class="text-right pr-0">
                                @if ($addon['amount_type'] == 'fixed')
                                    +{{ $invoice->formatCurrency($addon['amount']) }}
                                @else
                                    (+{{ $addon['addon_amount'] }}%) +{{ $invoice->formatCurrency($addon['amount']) }} 
                                @endif
                            </td>
                        </tr>
                    @endforeach
                @endif
                @if ($invoice->getCustomData()['package_discount'] != 0)
                    <tr>
                        <td colspan="{{ $invoice->table_columns - 2 }}" class="border-0"></td>
                        <td class="text-right pl-0">Package Discount</td>
                        <td class="text-right pr-0">
                            -{{ $invoice->formatCurrency($invoice->getCustomData()['package_discount']) }} 
                        </td>
                    </tr>
                @endif
                
                @if($invoice->getCustomData()['has_coupon_code_discount']))
                    @php
                        $cc = $invoice->getCustomData()['coupon_code'];
                    @endphp
                    <tr>
                        <td colspan="{{ $invoice->table_columns - 2 }}" class="border-0"></td>
                        <td class="text-right pl-0">Coupon Code ({{ $cc['code'] }}) </td>
                        <td class="text-right pr-0">
                            @if ($cc['amount_type'] == 'fixed')
                                -{{ $invoice->formatCurrency($cc['amount']) }}
                            @else
                                (-{{ $cc['coupon_amount'] }}%) -{{ $invoice->formatCurrency($cc['amount']) }}
                            @endif
                        </td>
                    </tr>
                @endif
